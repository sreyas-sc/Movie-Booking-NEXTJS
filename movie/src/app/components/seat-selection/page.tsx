...
const SeatSelection: React.FC = () => {
  const [seatLayout, setSeatLayout] = useState<string[][]>([]);
  const [selectedSeats, setSelectedSeats] = useState<Set<string>>(new Set());
  const [bookedSeats, setBookedSeats] = useState<Set<string>>(new Set()); // Track booked seats
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  const [selectedTheater, setSelectedTheater] = useState<string | null>(null);
  const [movieName, setMovieName] = useState<string | null>(null);
  const [movieId, setMovieId] = useState<string | null>(null);
  ...
  const handleSeatClick = (seatLabel: string) => {
    if (bookedSeats.has(seatLabel)) {
      return; // Ignore click on already booked seats
    }
    ...
    try {
      ...
      const options: RazorpayOptions = {
        ...
        handler: async (response: RazorpayPaymentResponse) => {
          ...
          const bookingData = await bookingResponse.json();
          if (bookingData) {
            setBookedSeats(new Set(Array.from(selectedSeats))); // Update booked seats
            Swal.fire({
              title: "Payment Done!",
              text: "The movie booking is completed",
              icon: "success"
  ...
  return (
    <Box>
      <Typography variant="h4" display="flex" justifyContent="center" alignItems="center" mb={2}>Select Your Seats</Typography>
      <Typography variant="body1" display="flex" justifyContent="center" alignItems="center">Selected Movie: {movieName}</Typography>
      <Typography variant="body1" display="flex" justifyContent="center" alignItems="center">Selected Date: {formatDate(selectedDate)}</Typography>
      <Typography variant="body1"display="flex" justifyContent="center" alignItems="center">Selected Theater: {getTheaterDetails(selectedTheater).name}</Typography>
      <Typography variant="body1" display="flex" justifyContent="center" alignItems="center" mt={10}>Total Amount: ₹{totalCost}</Typography>
            ...
            {row.map((seatLabel) => {
              ...
              return (
                <Grid item key={seatLabel}>
                  <Button
                    variant="outlined"
                    className={isBooked ? styles.disabled : (isSelected ? styles.selected : styles.available)}
                    onClick={() => handleSeatClick(seatLabel)}
      ...
      <Button  variant="contained" onClick={handleBookSeats} disabled={selectedSeats.size === 0}>
        Book Selected Seats
      </Button>
    </Box>
  );
